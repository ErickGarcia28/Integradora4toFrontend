    ...
    data.result.forEach(usuario => {
      ...
      $card_user.innerHTML = `
        <div class="card">
          <div class="card-content">
            ...
            <p><strong>Estado:</strong> ${usuario.status ? "Activo" : "Deshabilitado"}</p>
          </div>
          <div class="more-info">
            <button id="btn-status-${usuario.id}" class="btn-status ${usuario.status ? 'btn-red' : 'btn-green'}">
              ${usuario.status ? 'DESHABILITAR' : 'ACTIVAR'}
            </button>
            <button id="btn-editar${usuario.id}" class="btn-details"><a href="actUsuario.html">EDITAR</a></button>
          </div>
        </div>
      `;
      ...
      if(botonEstado){
        btnDetails.addEventListener("click", () => {
          ...
          console.log(`Categoría con id ${usuario.id} guardada en localStorage.`);
      });
      }
